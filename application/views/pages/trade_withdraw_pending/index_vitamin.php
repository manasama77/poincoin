...
<script>
	...
	function verifikasiTransfer(id, kode) {
		Swal.fire({
			...
			text: `Kamu akan memverifikasi bioner trade withdraw dengan kode ${kode}`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				$.ajax({
					url: `<?= site_url(); ?>admin/bioner_trade_withdraw/verifikasi`,
					method: 'post',
					dataType: 'json',
					data: {
						id: id
	...
	function deleteData(id) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: `<?= site_url(); ?>admin/bioner_trade_withdraw/delete`,
					method: 'post',
					data: {
						id: id
					},
