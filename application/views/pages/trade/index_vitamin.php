...
<script>
	...
	function verifikasiTransfer(id, kode) {
		Swal.fire({
			...
			text: `Kamu akan memverifikasi bioner trade dengan kode ${kode}`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				$.ajax({
					url: `<?= site_url(); ?>admin/bioner_trade/verifikasi_transfer`,
					method: 'post',
					dataType: 'json',
					data: {
						id: id
	...
	function deleteData(id) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: `<?= site_url(); ?>admin/bioner_trade/delete_trade`,
					method: 'post',
					data: {
						id: id
					},
