...
class PengajuanController extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('M_pengajuan_less', 'mless');
    }

    public function index()
    {
        $data['title']   = 'Pengajuan Management';
        $data['content'] = 'pengajuan/index';
        $data['vitamin'] = 'pengajuan/index_vitamin';
    ...
    {
        $no_kredit = $this->input->get('no_kredit');
        ...
        $arr = $this->mcore->get('pengajuan_barang', '*', ['no_kredit' => $no_kredit]);
        ...
        $data = [];
        foreach ($arr->result() as $k) {
            $id_kategori     = $k->id_kategori;
            $id_vendor       = $k->id_vendor;
            $id_jenis_barang = $k->id_jenis_barang;
            $id_barang       = $k->id_barang;
            $nama_barang     = $k->nama_barang;
            $spesifikasi     = $k->spesifikasi;
            $hpp             = number_format($k->hpp_pengajuan, 0);
            $tenor           = $k->tenor;
            $pokok           = number_format($k->pokok_pengajuan, 0);
            $margin_rp       = number_format($k->margin_rp_pengajuan, 0);
            $margin_persen   = $k->margin_persen_pengajuan;
            $harga_jual      = number_format($k->harga_jual_pengajuan, 0);
            $angsuran        = number_format($k->angsuran_pengajuan, 0);
...
/* End of file PengajuanController.php */
